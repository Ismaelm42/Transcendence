...
export async function renderRelations(relationsContainer: HTMLElement, userId: string): Promise<void> {
	...
	try {
		...
		await renderUserList(friends, friendsContainer, relationsContainer, userId);
...
async function getRelationsForUserId(userId: string): Promise<any[]> {
	try {
		const response = await fetch(`./back/get_all_friends_entries_from_an_id`, {
			method: "POST",
			headers: {
...
async function getUserById(userId: string): Promise<any> {
	try {
		const response = await fetch(`./back/get_user_by_id/?id=${userId}`, {
			method: "GET",
			headers: {
				"Content-Type": "application/json"
			}
...
async function renderUserList(relations: any[], container: HTMLElement, relationsContainer: HTMLElement, userId: string): Promise<void> {
	for (const relation of relations) {
		...
		userElement.innerHTML += (await userTemplate.text())
...
function getButtonConfig(status: string, relation: any, userId: string): string {
	switch (status) {
		case "accepted":
			return `
					<button type="button" class="btnRemoveFriend p-1.5 ms-2 text-sm font-medium text-white bg-red-700 rounded-lg border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" title="Remove friend">
					...
					<button type="button" class="btnBlockItem p-1.5 ms-2 text-sm font-medium text-white bg-indigo-700 rounded-lg border border-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800" title="Block user">
						<svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
		...
		case "pending":
			if (relation.userId == userId) {
				...
				return `
					<button type="button" class="btnCancelFriendRequest p-1.5 ms-2 text-sm font-medium text-white bg-yellow-700 rounded-lg border border-yellow-700 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800" title="Cancel request">
						<svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
				...
				return `
					<button type="button" class="btnAcceptRequest p-1.5 ms-2 text-sm font-medium text-white bg-green-700 rounded-lg border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" title="Accept request">
						<svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
					...
					<button type="button" class="btnDeclineRequest p-1.5 ms-2 text-sm font-medium text-white bg-red-700 rounded-lg border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" title="Decline request">
						<svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
		...
		case "blocked":
			return `
					<button type="button" class="btnUnblockItem p-1.5 ms-2 text-sm font-medium text-white bg-lime-700 rounded-lg border border-lime-700 hover:bg-lime-800 focus:ring-4 focus:outline-none focus:ring-lime-300 dark:bg-lime-600 dark:hover:bg-lime-700 dark:focus:ring-lime-800" title="Unblock user">
						<svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
