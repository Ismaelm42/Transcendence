...
const formOnePlayerHandler = () => {
    const p1email = document.getElementById('player1p-email');
    const p1password = document.getElementById('player1p-password');
    const p1insertButton = document.getElementById('btnPLayer1p');
    const guest1p = document.getElementById('btnguest1p');
    let habemusP1 = false;
    // Pulsamos guest1p
    guest1p === null || guest1p === void 0 ? void 0 : guest1p.addEventListener('click', (event) => {
    ...
    p1insertButton === null || p1insertButton === void 0 ? void 0 : p1insertButton.addEventListener('click', (event) => {
...
const asingOnePlayerGame = () => {
    const playOneP = document.getElementById('start-1p-game');
    if (!playOneP) {
        console.error('Play button for one player is missing in the DOM.');
        return;
    }
    playOneP.addEventListener('click', (event) => {
        event.preventDefault();
...
const asingTwoPlayerGame = () => {
    const playTwoP = document.getElementById('start-game');
    if (!playTwoP) {
        console.error('Play button for two players is missing in the DOM.');
        return;
    }
    playTwoP.addEventListener('click', (event) => {
        event.preventDefault();
